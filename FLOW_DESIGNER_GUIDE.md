...
1. **双击**连接线，弹出文本编辑框
2. 输入连接线文本（最多 50 字符）
3. 按 `Enter` 键或点击"保存"按钮保存
4. 文本会显示在连接线中间的白色标签上
5. **用途**：标注条件（如"批准"、"驳回"）或说明（如"满足条件"）
...
开始节点：绿色渐变背景 + 圆角矩形
处理节点：蓝色渐变背景 + 圆角矩形
决策节点：橙色渐变背景 + 菱形（clip-path polygon）
结束节点：红色渐变背景 + 圆角矩形
```
...
### Q4: 如何删除连接线？
**A**: 点击选中连接线，然后按 `Delete` 或 `Backspace` 键。
...
### Q7: 如何为连接线添加说明文字？
**A**: 双击连接线，会弹出文本输入框。输入文字后按 `Enter` 键或点击"保存"按钮。文字会显示在连接线中间的白色标签上。常用于标注分支条件（如"批准"、"驳回"）。
...
| 鼠标双击连接线 | 编辑连接线文本 | 弹出输入框，添加说明文字 |
| `Enter` | 保存文本 | 在连接线文本编辑框中按 Enter 保存 |
| `Escape` | 取消编辑 | 在连接线文本编辑框中按 Esc 取消 |
...
- **连接线组件**: `src/pages/FlowDesigner/components/CustomEdge.tsx`
- **工具栏**: `src/pages/FlowDesigner/components/Toolbar.tsx`
- **节点配置 Popover**: `src/pages/FlowDesigner/components/NodePopover.tsx`
- **连接线文本 Popover**: `src/pages/FlowDesigner/components/EdgeLabelPopover.tsx`
