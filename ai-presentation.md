...
### 什么是 LLM？
能理解和生成人类语言，完成各种复杂文本任务的人工智能模型，具备：
...
### 底层原理
- **基础架构**: Transformer（Google 2017年提出）
- **爆发点**: OpenAI 在2022年底将其引爆全世界
- **核心算法**: 预测下一个"字"（Next Token Prediction）
- **本质**: 庞大的数学函数，内部运行矩阵运算
...
### 定义
**Context = LLM 处理任务时接收的信息总和**

### 组成公式
```
Context = System Prompt      (立人设，定规矩)
        + User Prompt        (具体任务)
        + 历史对话
        + 当前输入
        + 注入的文档/数据
...
### Tokenizer - 翻译官
人类和 LLM 通过 **Tokenizer** 进行翻译：
- **编码**: 文字 → 数字（切分 + 映射）
- **解码**: 数字 → 文字（反向映射）

```
文本 → 切分为 Token → 映射为 Token ID (数字)
```
