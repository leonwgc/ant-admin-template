...
const mapList = (list, isOrg) => {
  return list.map((item) => {
    return {
      ...
      name: item.name,
      type: isOrg ? 'o' : 'p',
      dept: isOrg ? item.fullName : item.deptName,
    };
  });
};
 ...
 * @return {*}
 */
function PeopleDialogSelect(props: Props) {
  const {
    title = '请选择部门/员工',
    visible,
    value = [],
      ...
      <StyledWrap>
        ...
        <div className="right">
          {val.length > 0 && (
            <div>
              <div className="title">
                已选 {val.filter((v) => v.type === 'o').length}个部门{' '}
                {val.filter((v) => v.type === 'p').length}名员工
              </div>
                  ...
                  return (
                    <div className="list-item selected" key={o.id}>
                      <Space>
                        <StyledCircle
                          style={{
                            backgroundColor: o.type === 'o' ? '#65AFFF' : '#FAB20A',
                            color: '#fff',
                          }}
                        >
                          <Icon type={o.type === 'o' ? 'icon-bumen' : 'icon-yonghu'} />
                        </StyledCircle>
                        <div>
                          <div className="name">{o.name}</div>
                          <div className="dept">{o.dept}</div>
                        </div>
                      </Space>
...
export default React.memo(PeopleDialogSelect);
