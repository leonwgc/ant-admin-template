/**
 * @file locales/common/en.ts
 * @author leon.wang
 */

/**
 * Common translations shared across the application
 * Including menu items, common actions, and global UI text
 */
export default {
  // Common actions and UI
  switchLanguage: 'Switch Language',
  language: 'Language',

  // Menu translations
  menu: {
    jsFeature: 'JS Feature',
    intlNumberFormat: 'Intl.NumberFormat',
    elementHeight: 'Element Height',
    weakmap: 'WeakMap',
    proxy: 'Proxy',
    cssFeature: 'CSS Feature',
    cssNewFeatures: 'CSS New Features',
    cssRenderOptimization: 'CSS Render Optimization',
    cssGridLayout: 'CSS Grid Layout',
    cssFlexbox: 'CSS Flexbox',
    cssContainerQueries: 'CSS Container Queries',
    cssBlendModes: 'CSS Blend Modes',
    cssAnimation: 'CSS Animation',
    cssFilter: 'CSS Filter',
    users: 'Users',
    userList: 'User List',
    expandTable: 'Expand Table',
    addUser: 'Add User',
    editUser: 'Edit User',
    forms: 'Forms',
    responsiveForm: 'Responsive Form',
    dynamicList: 'Dynamic List',
    formValidation: 'Form Validation',
    virtualList: 'Virtual List',
    reactHooks: 'React Hooks',
    useTransition: 'useTransition',
    suspense: 'Suspense',
    useCountdown: 'useCountdown',
    verificationCodeCountdown: 'Verification Code Countdown',
    useResponsive: 'useResponsive',
    useDsTable: 'useDsTable',
    components: 'Components',
    dotStatus: 'DotStatus',
    verificationCode: 'VerificationCode',
    contactInfo: 'ContactInfo',
    userContactCard: 'UserContactCard',
    emailSuccessModal: 'EmailSuccessModal',
    verificationCodePage: 'VerificationCodePage',
    masonry: 'Masonry',
    chatbot: 'ChatBot',
    imageUpload: 'ImageUpload',
    fadeIn: 'FadeIn',
    dateTable: 'DateTable',
    libraries: 'Libraries',
    reactHookForm: 'React Hook Form',
    reactFormFieldHook: 'React Form Field Hook',
    zustand: 'Zustand',
    zustandKit: 'zustand-kit',
    sortableJs: 'Sortable.js',
    games: 'Games',
    tetris3d: '3D Tetris',
    flappyBirds3d: 'Flappy Birds 3D',
    snake3d: '3D Snake',
    match3: 'Match 3',
  },

  // Hotel booking common translations
  hotel: {
    address: 'Hotel Address',
    contactNumber: 'Contact Number',
    checkIn: 'Check-in',
    checkOut: 'Check-out',
    complete_zero: 'Complete (--night)',
    complete_one: 'Complete ({{count}} night)',
    complete_other: 'Complete ({{count}} nights)',
    selectDates: 'Please select check-in and check-out dates',
    night_zero: 'night',
    night_one: 'night',
    night_other: 'nights',
    room: 'Room',
    adult: 'Adult',
    searchRoom: 'Search Room',
    brandName: 'Yunshang Boutique Hotel',
    orderSearch: 'Order Search',
    roomTypesFound_zero: 'Found {{count}} room type',
    roomTypesFound_one: 'Found {{count}} room type',
    roomTypesFound_other: 'Found {{count}} room types',
    payOnArrival: 'Pay at Hotel',
  },

  // Guest information (used in booking flow)
  guestInfo: {
    title: 'Guest Information',
    contactInfo: 'Contact Information',
    roomCountSuffix_one: 'room',
    roomCountSuffix_other: 'rooms',
    nameLabel: 'Guest Name',
    lastNamePlaceholder: 'Last Name',
    firstNamePlaceholder: 'First Name',
    phoneLabel: 'Mobile Number',
    sendCode: 'Send Code',
    phoneTip: 'Only mainland China mobile numbers are supported',
    codeLabel: 'Verification Code',
    required: '*',
    reSendCodeCount: 'Resend Code ({{count}})',
    resendCode: 'Resend Code',
  },
};
